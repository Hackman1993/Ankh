...
namespace ankh::html::css3::parser {
  ...
  auto single_selector_parser_ = rule<selector_parser_id, ast::css_single_element>{"SingleSelectorParser"} = checked_selector_parser_ | enabled_selector_parser_ | only_of_type_selector_parser_ | empty_selector_parser_ | last_of_type_selector_parser_ | first_of_type_selector_parser_ | last_child_selector_parser_ | last_nth_child_selector_parser_ | nth_child_selector_parser_ | attribute_exists_parser_ | attribute_selector_parser_ | class_selector_parser_ | html_selector_parser_ | id_selector_parser_;
  //auto multi_selector = rule<selector_parser_id, ast::
  auto css3_selector_group_parser_ = rule<selector_parser_id, std::vector<ast::css_single_element>>{"SelectorGroupParser"} = *single_selector_parser_ >> *(operator_parser_ > single_selector_parser_);

  auto css3_selector_parser_ = rule<selector_parser_id, std::vector<ast::css_single_element>>{"CSS3Parser"} = css3_selector_group_parser_;



  std::unique_ptr<std::vector<ast::css_single_element>> parse_ast(std::string source) {
    ...
    std::unique_ptr<std::vector<ast::css_single_element>> css_ast = std::make_unique<std::vector<ast::css_single_element>>();
    auto const parser = boost::spirit::x3::no_skip[boost::spirit::x3::with<boost::spirit::x3::error_handler_tag>(
        std::ref(error_handler))[css3_selector_parser_]];
  ...
  std::unordered_map<html_element *, std::reference_wrapper<html_element>> execute_query_(std::vector<ast::css_single_element> &css_ast,
                                                                                          const std::unordered_map<html_element *, std::reference_wrapper<html_element>> &input_elements) {
    ...
    for (auto &selector: css_ast) {
      ...
      css3::selector_visitor visitor(next_input);
